...
export async function seedLocalScenario(prisma, env = process.env) {
  ...
  return prisma.$transaction(async tx => {
    ...
    for (const [id, status, itemId, sku, quantity, receivedQuantity] of [
      ['LOCAL-DEMO-PO-001', 'partially_received', 'LOCAL-DEMO-ITEM-001', 'LDM-001', 50, 20],
      ['LOCAL-DEMO-PO-002', 'open', 'LOCAL-DEMO-ITEM-002', 'LDM-002', 40, 0],
    ]) {
      await tx.purchaseOrder.upsert({
        where: { id },
        create: { id, tenantId, status, supplierId: 'LOCAL-DEMO-SUP-001', supplierName: '本地演示供应商 A', sourceRequestId: 'LOCAL-DEMO-PR-001', amount: quantity * 100, currency: 'CNY', owner: 'Local Demo', priority: id.endsWith('002') ? 'high' : 'medium', metadata, lines: { create: [{ id: `${id}-LINE-001`, itemId, sku, itemName: sku === 'LDM-001' ? '本地演示控制器' : '本地演示传感器', orderedQuantity: quantity, receivedQuantity, unit: 'pcs', unitPrice: 100, amount: quantity * 100, metadata }] } },
        update: {},
      })
    }
