...
const reportCopy = (label: string, language = typeof document === "undefined" ? "en-US" : document.documentElement.lang) => {
  ...
  return REPORT_ENGLISH[label] || workspaceCopy(label, language);
};
...
export function BiDashboard({ view, onNavigate: _onNavigate }: { view: DashboardView; onNavigate?: NavigateFn }) {
  ...
  return <div className="space-y-3" data-testid="bi-dashboard" data-view={view}>
    ...
    {!loading && !error && report && <>
      ...
      <Card><div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: `1px solid ${A.border}` }}><div><h2 className="text-sm font-semibold">{copy(VIEW_COPY[view].detailTitle)}</h2><p className="mt-1 text-[11px]" style={{ color: A.sub }}>{report.details.length} {copy("条筛选后明细，与 KPI、图表和导出口径一致")}</p></div><button onClick={exportCurrent} className="text-xs" style={{ color: A.blue }}><Download size={12} className="inline mr-1" />{copy("导出五表工作簿")}</button></div>{report.details.length ? <div className="overflow-x-auto"><table className="w-full min-w-[880px] text-xs"><thead><tr>{displayedColumns.slice(0, 11).map((column) => <th key={column.key} className="px-4 py-2 text-left" style={{ color: A.gray1 }}>{copy(column.label)}</th>)}</tr></thead><tbody>{report.details.slice(0, 15).map((row, index) => <tr key={String(row.id || index)} style={{ borderTop: `1px solid ${A.border}` }}>{displayedColumns.slice(0, 11).map((column) => <td key={column.key} className="px-4 py-2 whitespace-nowrap">{column.key === "id" ? <BusinessEntityLink entityType={view === "sales" ? "sales_order" : view === "inventory" ? "item" : view === "finance" ? "supplier_invoice" : "purchase_order"} entityId={String(row[column.key] || "")}>{String(row[column.key] || "—")}</BusinessEntityLink> : column.type === "currency" ? formatMetric(Number(row[column.key] || 0), "currency", typeof row.currency === "string" ? row.currency : report.dataScope.currencyCode) : String(row[column.key] ?? "—")}</td>)}</tr>)}</tbody></table></div> : <div className="p-8 text-center text-xs" style={{ color: A.sub }}>{copy("当前范围暂无真实业务记录。可以调整筛选、创建业务数据，或在本地加载演示场景。")}</div>}</Card>
    </>}
  </div>;
}
