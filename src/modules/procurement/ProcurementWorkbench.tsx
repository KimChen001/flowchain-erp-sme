...
export function ProcurementWorkbench({ onNavigate }: { onNavigate?: ProcurementNavigate }) {
  ...
  return <div className="space-y-4"><Card className="p-5"><div className="flex items-center justify-between"><div><h1 className="text-lg font-semibold">{language === "en-US" ? `Today’s purchasing work: ${rows.length}` : `今日采购待办：${rows.length}`}</h1><p className="text-xs" style={{ color: A.sub }}>{language === "en-US" ? "Purchase requests, approvals, open orders, and fulfillment records come from the purchasing runtime repository." : "采购申请、审批、开放及履约中的 PO 均来自采购 runtime repository。"}</p></div><div className="flex gap-2"><button onClick={() => void load()} className="inline-flex items-center gap-1 rounded border px-3 py-2 text-xs"><RefreshCw size={14}/>{copy("刷新")}</button><button onClick={() => onNavigate?.("procurement:requests")} className="rounded bg-blue-600 px-3 py-2 text-xs text-white">{copy("新建采购申请")}</button></div></div></Card><div className="grid grid-cols-2 gap-3 md:grid-cols-3">{[{ id: "all" as const, label: "全部待办", value: rows.length }, { id: "approval" as const, label: "待我审批", value: rows.filter(row => row.bucket === "approval").length }, { id: "tracking" as const, label: "跟进中", value: rows.filter(row => row.bucket === "tracking").length }].map(item => <button key={item.id} onClick={() => setFilter(item.id)} className="rounded border bg-white p-4 text-left"><div className="text-xs">{copy(item.label)}</div><div className="text-2xl font-semibold">{item.value}</div></button>)}</div><Card className="overflow-hidden"><div className="border-b p-4 text-sm font-semibold">{copy("待办队列")}</div>{state === "loading" ? <div className="py-12 text-center text-xs">{copy("加载中")}</div> : shown.length === 0 ? <div className="py-12 text-center text-sm" style={{ color: A.sub }}>{copy("暂无采购事项")}</div> : <table className="w-full text-xs"><tbody>{shown.map(row => <tr key={row.id} className="border-t"><td className="p-3">{copy(row.type)}</td><td className="p-3 text-blue-600"><EntityLink kind={row.kind} id={row.id}>{row.id}</EntityLink></td><td className="p-3"><div>{row.status}</div>{row.signals?.length ? <div className="mt-1 flex flex-wrap gap-1">{row.signals.map(signal => <span key={signal} className="rounded bg-amber-50 px-2 py-0.5 font-semibold text-amber-700">{copy(signal)}</span>)}</div> : null}</td><td className="p-3">{row.amount}</td></tr>)}</tbody></table>}</Card></div>;
}
