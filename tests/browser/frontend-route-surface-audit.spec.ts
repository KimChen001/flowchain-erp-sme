...
test("163/163 frontend route stability audit resolves without crashes, route 404s, or API 5xx responses", async ({
  page,
  request,
}) => {
  ...
  expect(
    failures,
    `${appRouteRegistry.length}/163 frontend route stability audit; business semantics are covered separately`,
  ).toEqual([]);
});
