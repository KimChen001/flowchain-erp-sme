...
function baseContext(pathname, body) {
  ...
  return {
    ctx: {
      req: { method: 'POST', headers: {} }, res: {}, url: { pathname }, db: structuredClone(db), repositories: { mode: 'json' }, identity: { authenticated: true, tenantId: 'tenant-test', userId: 'user-test', role: 'manager' }, env: { FLOWCHAIN_WORKSPACE_TIMEZONE: 'Asia/Shanghai' },
      aiBusinessQueryActor: actor, aiBusinessQuerySummaryService: summaryService,
      readBody: async () => structuredClone(body), send: (_res, status, next) => { payload = { status, payload: next } },
