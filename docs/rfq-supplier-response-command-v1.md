...
The governed internal UI documented in `docs/rfq-supplier-response-ui-v1.md` consumes these commands without adding another business authority. This version still adds no Supplier Portal, email delivery, external identity, public submission, scoring, award, approval, PO conversion, AI mutation, automatic communication, deletion, or overwrite. The read-only RFQ Supplier Comparison contract is documented separately; it does not rank or recommend suppliers.
